...
namespace GTKMapDemo.Drawing
{
    ...
    class GTKImage : IImage
    {
        ...
        {
            GTKImage netImage = new GTKImage {_image = new Bitmap(stream)};
            netImage._image.SetResolution(96, 96);
            return netImage;
        }
        ...
        {
            GTKImage netImage = new GTKImage {_image = new Bitmap(width, height)};
            netImage._image.SetResolution(96, 96);
            for (int i = 0; i < width; i++)
            {
                for (int j = 0; j < height; j++)
                {
                    netImage._image.SetPixel(j, i, System.Drawing.Color.FromArgb(rgb[i * height + j]));
                }
            }
            return netImage;
        }
        ...
        public static IImage CreateImage(byte[] bytes,int offset,int len)
        {
            ...
            GTKImage netImage = new GTKImage {_image = new Bitmap(memoryStream)};
            netImage._image.SetResolution(96, 96);
            return netImage;
        ...
        {
            GTKGraphics netGraphics = new GTKGraphics(Graphics.FromImage(_image));
